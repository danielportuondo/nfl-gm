...
async function main(): Promise<void> {
  const strictness = (process.argv[2] ?? 'balanced') as 'lenient' | 'balanced' | 'strict' | 'ruthless'
  ...
  for (const ai of aiTeams(state).slice(0, 16)) {
    const theirs = state.teams[ai]!.roster.map((r) => r.playerId).sort((a, b) => state.scouting[b]!.ovr - state.scouting[a]!.ovr)
    for (let i = 0; i < 12; i++) {
      for (let j = i; j < Math.min(theirs.length, i + 6); j++) {
        const proposal: TradeProposal = {
          ...
          initiatedBy: 'USER', season: state.season, week: state.week,
        }
        const ev = ctx.modules.trade.evaluate(state, proposal, ctx)
        if (!ev.valid) { bump('invalid'); continue }
        if (ev.p >= 0.5) { bump('p>=0.5 (no counter by design)'); continue }
        const shortfall = ev.valueOut + ev.needAdj + ev.margin - ev.valueIn
        if (shortfall > acceptanceConstants.counterMaxShortfallPct * Math.max(1, ev.valueOut)) { bump('shortfall > maxPct'); continue }
        ...
        if (out.accepted) { bump('accepted by luck'); continue }
        bump(out.counter ? 'COUNTERED' : 'eligible but no counter')
        if (!out.counter && (exits['eligible but no counter'] ?? 0) <= 3) {
          console.log('  no-counter example', { p: ev.p.toFixed(3), valueIn: ev.valueIn.toFixed(1), valueOut: ev.valueOut.toFixed(1), needAdj: ev.needAdj.toFixed(1), margin: ev.margin.toFixed(1), shortfall: shortfall.toFixed(1) })
        }
      }
    }
  }
