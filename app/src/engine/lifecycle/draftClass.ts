...
import {
  POSITIONS,
  type CurvesFile, type GeneratedClass, type LeagueState, type PlayerId, type Position, type Prospect,
  type Rng, type ScoutingView, type Season, type TrueTrajectory,
} from '@contracts/index'
import {
  BUST_MAX_CAREER_YEARS, BUST_PERCENTILE_CAP, CLASS_SIZE_JITTER, DRAFT_SLOT_NOISE_SCALE,
  OUTCOME_YEAR_NOISE_SD, ROOKIE_AGE_MAX, ROOKIE_AGE_MIN,
  ROOKIE_CONFIDENCE_DRAFTED, ROOKIE_CONFIDENCE_UDFA, UDFA_POT_NOISE_SD, UDFA_POT_QUANTILE_LEVELS,
} from './constants'
import { bucketForPick, clampRating, findOutcomeTable, interpolateAtLevel, interpolateSlotGrade, outcomeValueAt } from './curves'
...
function sampleTrajectory(
  ...
  const careerYears = bust ? Math.min(maxYears, rng.fork(`${id}:bustYears`).int(1, BUST_MAX_CAREER_YEARS)) : maxYears
...
function buildProspect(
  ...
  const scouting = pick != null ? draftedConsensus(curves, pick, rng, id) : udfaConsensus(curves, rng, id)
...
export function generateDraftClass(state: LeagueState, curves: CurvesFile, rng: Rng): GeneratedClass {
  ...
  const order = [...prospects]
    .sort((a, b) => b.scouting.pot - a.scouting.pot || b.scouting.ovr - a.scouting.ovr || a.id.localeCompare(b.id))
    .map((p) => p.id)
