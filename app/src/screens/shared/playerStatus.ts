...
const CURRENT_SEASON_ROOKIE_PHASES = new Set<LeagueState['phase']>([
  'PRESEASON',
  'REGULAR',
  'PLAYOFFS',
  'OFFSEASON_RESIGN',
 ...
 */
export function isRookie(
  player: Pick<Player, 'rookieSeason'>,
  state: Pick<LeagueState, 'season' | 'phase'>,
): boolean {
  const targetSeason = CURRENT_SEASON_ROOKIE_PHASES.has(state.phase)
    ? state.season
    : state.season + 1
