...
export const PositionSchema = z.enum(POSITIONS).describe('Position group. Fine-grained nflverse positions are collapsed in the pipeline (T/G/C→OL, DE/DT/NT→DL, OLB/ILB/MLB→LB, FS/SS→S).')

export const SeasonSchema = z.number().int().min(1920).max(2200).describe('NFL season year, e.g. 2015.')
export const WeekSchema = z.number().int().min(0).max(30)
export const TeamIdSchema = z.string().min(2).max(3).describe('Canonical franchise id (current abbreviation, e.g. LAR for STL/LA). See teams.ts.')
export const PlayerIdSchema = z.string().min(1).describe('gsis_id for real players; "gen-<seed>-<n>" for procedural players.')
...
export const ATTRIBUTION = 'Data courtesy of nflverse (CC BY 4.0). Unofficial fan-made project; not affiliated with the NFL.'
...
export const ScoutingViewSchema = z.object({
  ...
  pot: RatingSchema.describe('Consensus ceiling from age/draft slot/combine. NEVER derived from real future.'),
  confidence: ProbabilitySchema.describe('0–1. Low for rookies/UDFA, high for established veterans.'),
})

export const TrueTrajectorySchema = z.object({
  bySeason: z.record(z.string(), RatingSchema).describe('Keys are season years as strings. True value per season from the real career (or generated once past data).'),
  retiresAfter: SeasonSchema.nullable().describe('Last season played; null = unknown/still active.'),
})
...
export const TeamStateSchema = z.object({
  ...
  tradeAnnoyance: z.number().min(0).describe('Raised by declined lowballs; raises this team\'s trade margin for the season (§6.5).'),
...
export const DraftPickSchema = z.object({
  season: SeasonSchema,
  round: z.number().int().min(1).max(7),
  pick: z.number().int().min(1).max(300).nullable().describe('Overall pick number; null until the draft order for that season is set.'),
...
export const PlayerGameLineSchema = z.object({
  playerId: PlayerIdSchema,
  teamId: TeamIdSchema,
  passAtt: z.number().int().optional(), passCmp: z.number().int().optional(), passYds: z.number().int().optional(),
  passTd: z.number().int().optional(), passInt: z.number().int().optional(),
  rushAtt: z.number().int().optional(), rushYds: z.number().int().optional(), rushTd: z.number().int().optional(),
  targets: z.number().int().optional(), rec: z.number().int().optional(), recYds: z.number().int().optional(), recTd: z.number().int().optional(),
  tackles: z.number().int().optional(), sacks: z.number().optional(), ints: z.number().int().optional(),
...
export const StandingRowSchema = z.object({
  teamId: TeamIdSchema,
  wins: z.number().int(), losses: z.number().int(), ties: z.number().int(),
  pct: z.number().min(0).max(1),
  pointsFor: z.number().int(), pointsAgainst: z.number().int(),
...
export const PlayoffSeedSchema = z.object({ teamId: TeamIdSchema, conf: z.enum(['AFC', 'NFC']), seed: z.number().int().min(1).max(7) })
...
export const PHASES = ['PRESEASON', 'REGULAR', 'PLAYOFFS', 'OFFSEASON_RESIGN', 'DRAFT', 'UDFA', 'FREE_AGENCY', 'TRAINING_CAMP'] as const
export const PhaseSchema = z.enum(PHASES).describe('Season phases in order. After TRAINING_CAMP the season increments and PRESEASON begins.')
...
export const DraftRoomStateSchema = z.object({
  ...
  order: z.array(DraftPickSchema).describe('All picks of the season in order; playerId filled as they are made.'),
  ...
  pendingOffers: z.array(TradeProposalSchema).describe('AI offers shown while the user is on the clock.'),
})
...
export const SavedLeagueSchema = z.object({
  ...
  truth: z.record(PlayerIdSchema, TrueTrajectorySchema).describe('HIDDEN. Present in state; nothing under screens/ or ui/ may read it.'),
  ...
  divergence: z.array(PlayerIdSchema).describe('Players whose ownership left the historical path (§6.8).'),
...
export const TeamInfoSchema = z.object({
  ...
  colors: z.object({ primary: HexColorSchema, secondary: HexColorSchema, tertiary: HexColorSchema.optional() }),
  aliases: z.array(z.string()).describe('nflverse team codes this franchise has used (e.g. ["STL","LA","LAR"]). No logo URLs — ever.'),
  eras: z.array(z.object({ from: SeasonSchema, to: SeasonSchema.nullable(), city: z.string(), name: z.string(), abbr: z.string() })).optional(),
})
...
export const CapFileSchema = z.object({
  attribution: AttributionSchema,
  bySeason: z.record(z.string(), z.number().positive()).describe('Salary cap in $M keyed by season string. 2010 uncapped → 123.0 for game purposes.'),
  growthAfterData: z.number().min(0).max(0.2).describe('Annual growth applied beyond the last real season (0.06).'),
})

export const AgingCurveSchema = z.object({
  pos: PositionSchema,
  byAge: z.record(z.string(), z.number()).describe('Expected true-value delta entering a season at this age (ages as string keys).'),
...
export const OutcomeTableSchema = z.object({
  bucket: z.string().describe('Pick-slot bucket label, e.g. "1-10", "11-32", "33-64", "65-105", "106-160", "161-260", "UDFA".'),
...
export const RetirementCurveSchema = z.object({
  pos: PositionSchema,
  byAge: z.record(z.string(), ProbabilitySchema).describe('P(retire after season) at this age, evaluated at position-mean value.'),
  valueSlope: z.number().describe('Logit slope per rating point above/below position mean (negative: better players retire later).'),
})
...
export const CurvesFileSchema = z.object({
  ...
  slotGrade: z.array(SlotGradePointSchema).describe('Consensus ovr/pot by overall pick; interpolate between points.'),
  udfaGrade: z.object({ ovrMean: RatingSchema, ovrSd: z.number().min(0), potQuantiles: z.array(RatingSchema) }),
  outcomes: z.array(OutcomeTableSchema),
  retirement: z.array(RetirementCurveSchema),
  positionMix: z.record(PositionSchema, ProbabilitySchema).describe('Share of a draft class by position; sums to ~1.'),
  classSize: z.object({ drafted: z.number().int(), udfa: z.number().int() }),
  names: z.object({ first: z.array(z.string()).min(50), last: z.array(z.string()).min(50) }).describe('Generated names for procedural players. Never real player names.'),
})

export const InjuryModelFileSchema = z.object({
  attribution: AttributionSchema,
  fitSeasons: z.tuple([SeasonSchema, SeasonSchema]),
  ratePerPlayerGame: z.record(PositionSchema, ProbabilitySchema).describe('P(new multi-week injury) per active player per game.'),
  duration: z.array(z.object({ weeks: z.number().int().min(1), p: ProbabilitySchema })).describe('Discrete distribution of weeks out; sums to ~1.'),
  kinds: z.array(z.object({ kind: z.string(), p: ProbabilitySchema })),
  permanentLoss: z.object({ minWeeks: z.number().int(), p: ProbabilitySchema, lossRange: z.tuple([z.number(), z.number()]) }),
})
...
export const ManifestSchema = z.object({
  ...
  latestRealSeason: SeasonSchema.describe('Last season with complete real data; procedural generation begins after it.'),
  sizesBytes: z.record(z.string(), z.number().int()).optional().describe('gzipped size per file, for the CI budget check.'),
})
...
export const SeasonPlayerSchema = PlayerSchema.extend({
  ...
  trueValue: RatingSchema.describe('True value for THIS season only. Loaded into truth by the engine; never shown.'),
...
export const RosterEntrySchema = z.object({
  playerId: PlayerIdSchema,
  apy: z.number().min(0).optional().describe('Real APY hint in $M when known (contracts data ~2011+).'),
...
export const DraftOrderEntrySchema = z.object({
  ...
  originalTeam: TeamIdSchema.describe('Team the pick originally belonged to (best effort; = team when unknown).'),
...
export const SeasonDraftFileSchema = z.object({
  ...
  prospects: z.array(ProspectSchema).describe('Every drafted player + every real UDFA of the class.'),
